...
export default function RegisterPage() {
  ...
  const handleRegister = async () => {
    try {
      setLoading(true);
      setError("");
      const res = await api.post('/auth/register', { fullName: name, email, password: pw });
  ...
  const handleGoogleLogin = () => {
    const backendUrl = process.env.NEXT_PUBLIC_API_URL || 'https://lapangin-backend.up.railway.app';
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0F172A] to-[#071810] flex items-center justify-center px-4 pt-20 pb-10">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-sm">Join thousands of futsal enthusiasts</p>
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input value={name} onChange={e => setName(e.target.value)} placeholder="Rizki Pratama"
          ...
          <div>
            ...
            <div className="relative">
              <input value={pw} onChange={e => setPw(e.target.value)} type={showPw ? "text" : "password"} placeholder="Min. 8 characters"
        ...
        <GreenButton onClick={handleRegister} className="w-full py-3.5 mt-6 text-base" type="submit">
          {loading ? "Creating Account..." : "Create Account"}
        </GreenButton>

        <p className="text-center text-gray-400 text-sm mt-4">
          By registering you agree to our{" "}
          <span className="text-[#4ADE80] hover:underline cursor-pointer">Terms</span>{" "}and{" "}
          <span className="text-[#4ADE80] hover:underline cursor-pointer">Privacy Policy</span>
        </p>
        <p className="text-center text-gray-400 text-sm mt-4">
          Already have an account?{" "}
          <Link href="/login" className="text-[#4ADE80] font-semibold hover:text-[#22C55E] transition-colors">Sign in</Link>
        </p>
      </motion.div>
    </div>
  );
